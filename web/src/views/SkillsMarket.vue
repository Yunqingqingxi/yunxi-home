<template>
  <div class="market-page">
    ...
    <div class="tab-bar">
      <button :class="['tab', { active: tab === 'skills' }]" @click="tab = 'skills'">⭐ 技能 Skills</button>
      <button :class="['tab', { active: tab === 'mcp' }]" @click="tab = 'mcp'">🔌 MCP 工具</button>
      <button :class="['tab', { active: tab === 'installed' }]" @click="tab = 'installed'; loadInstalled()">📦 已安装</button>
    </div>
    ...
    <div v-if="tab === 'skills'" class="tab-content">
      <div class="action-bar">
        <div class="search-wrap" style="flex:1">
          <svg width="13" height="13" viewBox="0 0 13 13" fill="none" stroke="currentColor" stroke-width="1.6"><circle cx="5.5" cy="5.5" r="3.5"/><path d="M8.5 8.5L12 12"/></svg>
          <input v-model="skillSearch" placeholder="搜索技能，如 backup, monitor, docker..."
            class="search-inp" style="flex:1" @keyup.enter="searchSkills" />
        </div>
        <button class="btn-accent" @click="searchSkills" :disabled="searching">
          {{ searching ? '搜索中...' : '搜索' }}
        </button>
      </div>

      <div v-if="skillResults.length" class="card-grid">
        <div v-for="s in skillResults" :key="s.name" class="card-item" :class="{ installed: s._installed }">
          <div class="card-head">
          ...
          <button v-if="s._installed" class="card-action done">✓ 已安装</button>
          <button v-else class="card-action accent" @click="installSkill(s)" :disabled="installing === s.name">
            {{ installing === s.name ? '安装中...' : '下载安装' }}
          </button>
        </div>
      </div>
      <div v-else-if="skillSearched && !searching" class="empty-state">
        <p>未找到匹配的技能</p>
        <span class="hint">尝试其他关键词</span>
      </div>
      <div v-else-if="!searching" class="hint-banner">
        <p>🔍 输入关键词搜索在线技能市场<br>
        <span style="font-size:11px;color:var(--text-muted)">来源：GitHub 开源社区 + 内置推荐</span></p>
      </div>
    </div>

    <!-- ── MCP Market ── -->
    <div v-if="tab === 'mcp'" class="tab-content">
      <div v-if="installError" class="install-banner error">{{ installError }} <button @click="installError=''">✕</button></div>
      <div v-if="installSuccess" class="install-banner success">✅ {{ installSuccess }} 安装成功！已重载 MCP 工具。 <button @click="installSuccess=''">✕</button></div>
      <div class="section">
        <h4>🔥 热门推荐</h4>
        <div class="card-grid">
          <div v-for="m in popularMCP" :key="m.package" class="card-item" :class="{ installed: m._installed }">
            <div class="card-head">
              <span class="card-name">{{ m.name }}</span>
              <span class="card-cat">{{ m.category }}</span>
            </div>
            <div class="card-desc">{{ m.description }}</div>
            <div class="card-meta">{{ m.package }}</div>
            <button v-if="m._installed" class="card-action done">✓ 已安装</button>
            <button v-else class="card-action accent" @click="installMCP(m.package)" :disabled="installing === m.package">
      ...
      <div class="section">
        ...
        <div class="action-bar">
          <div class="search-wrap" style="flex:1">
            <input v-model="mcpSearch" placeholder="如 filesystem, github, puppeteer..."
              class="search-inp" style="flex:1" @keyup.enter="searchMCP" />
          </div>
          <button class="btn-accent" @click="searchMCP" :disabled="searching">搜索</button>
        </div>
        <div v-if="mcpResults.length" class="card-grid" style="margin-top:10px">
          <div v-for="r in mcpResults" :key="r.name" class="card-item" :class="{ installed: r._installed }">
            <div class="card-head">
              <span class="card-name">{{ r.name }}</span>
              <span v-if="r.score" class="card-cat">{{ r.score }}% 匹配</span>
            </div>
            <div class="card-desc">{{ r.desc }}</div>
            <button v-if="r._installed" class="card-action done">✓ 已安装</button>
            <button v-else class="card-action accent" @click="installMCP(r.name)" :disabled="installing === r.name">安装</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="tab === 'installed'" class="tab-content">
      <div v-if="installedSkills.length" class="section">
        <h4>⭐ 已安装技能 ({{ installedSkills.length }})</h4>
        <div class="card-grid">
          <div v-for="s in installedSkills" :key="s.name" class="card-item">
            <div class="card-head"><span class="card-name">/{{ s.name }}</span></div>
            <div class="card-desc">{{ s.description }}</div>
            <button class="card-action" @click="runSkill(s.name)">执行</button>
          </div>
        </div>
      </div>
      <div v-if="installedMCPServers.length" class="section">
        <h4>🔌 已安装 MCP ({{ installedMCPServers.length }})</h4>
        <div class="card-grid">
          <div v-for="s in installedMCPServers" :key="s.name" class="card-item">
            <div class="card-head"><span class="card-name">{{ s.name }}</span></div>
            <div class="card-desc">{{ s.command }} {{ (s.args||[]).join(' ') }}</div>
          </div>
        </div>
      </div>
      <div v-if="!installedSkills.length && !installedMCPServers.length" class="empty-state">
        <p>暂无已安装项</p>
      </div>
    </div>

    <!-- MCP Config Modal -->
    <div v-if="showMCPConfig" class="modal-overlay" @click.self="showMCPConfig = false">
      <div class="modal-card">
        <h4>🔧 配置 {{ mcpConfigPkg }}</h4>
        <p style="font-size:12px;color:var(--text-muted)">此 MCP 服务器需要以下参数才能运行：</p>
        <div v-for="p in mcpConfigParams" :key="p.name" class="config-field">
          <label>{{ p.label }} <span v-if="p.required" style="color:var(--color-danger)">*</span></label>
          <input v-model="mcpConfigValues[p.name]" :placeholder="p.description" class="modal-input"
            :value="mcpConfigValues[p.name] || p.default || ''" />
          <span class="config-hint">{{ p.description }}</span>
        </div>
        <div class="modal-actions">
          <button class="btn-cancel" @click="showMCPConfig = false; installing = null">取消</button>
          <button class="btn-ok" @click="submitMCPConfig">安装</button>
        </div>
      </div>
    </div>

    <!-- MCP Install Progress -->
    <MCPInstallProgress :tasks="installTasks" @clear="installTasks = installTasks.filter(t => t.status === 'running')" />

    <!-- Result Modal -->
    <div v-if="resultModal" class="modal-overlay" @click.self="resultModal = ''">
      <div class="modal-card">
        <h4>{{ resultTitle }}</h4>
        <pre class="install-result">{{ resultModal }}</pre>
        <div class="modal-actions"><button class="btn-ok" @click="resultModal = ''">确定</button></div>
      </div>
    </div>
  </div>
</template>
