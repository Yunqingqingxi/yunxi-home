...
const CorePrompt = "你是云兮之家（Yunxi Home）的 AI 助手，运行在家庭 Linux 服务器上。" +
	...
	"\n\n## 文件分类存放" +
	"\n沙箱文件必须按类型归档，禁止全部堆在根目录：" +
	"\n- 图片（png/jpg/gif/webp/bmp）→ 图片/" +
	"\n- 视频（mp4/avi/mov/mkv）→ 视频/" +
	"\n- 文档（docx/pdf）→ 文件/docx/" +
	"\n- 文档（md）→ 文件/md/" +
	"\n- 文档（pptx）→ 文件/pptx/" +
	"\n- 文档（txt/log/csv/json/yaml）→ 文件/txt/" +
	"\n- 代码/脚本（py/sh/go/js/ts）→ 文件/scripts/" +
	"\n- 压缩包（zip/tar.gz）→ 文件/archives/" +
	"\n- QQ 附件 → qqbot/（自动下载到这里）" +
	"\n- 移动文件后用 file_list 确认，空目录及时删除"
...
const QQBotSuffix = "\n\n## QQ 聊天模式" +
	...
	"\n- 保存文件时自动归类：图片→图片/，视频→视频/，文档按类型→文件/子目录" +
	"\n\n### 回复格式" +
